...
def fetch_blinkit_live(query: str, lat: str = "13.0470976", lon: str = "77.5476596"):
    ...
    raw_api = f"https://blinkit.com/v1/layout/search?q={encoded_q}"
    
    # Try direct endpoint first, then proxy fallbacks to bypass foreign cloud IP filtering
    endpoints = [
        raw_api,
        f"https://corsproxy.io/?url={urllib.parse.quote(raw_api)}",
        f"https://api.allorigins.win/raw?url={urllib.parse.quote(raw_api)}"
    ]
    ...
    for ep in endpoints:
        try:
            r = requests.get(ep, headers=headers, impersonate="chrome124", timeout=12)
            if r.status_code == 200:
                ...
                for w in widgets:
                    products = w.get("data", {}).get("products", [])
                    for product in products:
                        title = product.get("name", "").strip()
                        if not title:
                            continue

                        is_ad = bool(product.get("is_sponsored", False) or product.get("ad_id") is not None)
...
def fetch_zepto_live(query: str, lat: str = "13.0470976", lon: str = "77.5476596"):
    results = []
    encoded_q = urllib.parse.quote(query.strip())
    raw_api = f"https://api.zeptonow.com/api/v3/search?query={encoded_q}&pageNumber=1&mode=MANUAL"
            ...
            if r.status_code == 200:
                ...
                for block in layout:
                    ...
                    for item in items:
                        ...
                        is_ad = bool(item.get("isSponsored", False) or product.get("isSponsored", False))
...
def fetch_instamart_live(query: str, lat: str = "13.0470976", lon: str = "77.5476596"):
    results = []
    encoded_q = urllib.parse.quote(query.strip())
    raw_api = f"https://www.swiggy.com/api/instamart/search?query={encoded_q}&lat={lat}&lng={lon}"
    ...
    for ep in endpoints:
        try:
            ...
            if r.status_code == 200:
                ...
                for c in cards:
                    nodes = c.get("data", {}).get("nodes", [])
                    for prod in nodes:
                        title = prod.get("display_name", "").strip()
                        if not title:
                            continue

                        is_ad = bool(prod.get("is_sponsored", False) or (prod.get("ad_info") is not None))
